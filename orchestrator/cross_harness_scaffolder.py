...
PROTOCOL_VERSION = "Cross-Harness Scaffolder v1.0.0"
...
create table if not exists cross_harness_validations (
  ...
  session_id text not null references cross_harness_sessions(session_id),
  item_id text not null references cross_harness_items(item_id),
...
create index if not exists cross_harness_round_lookup
  on cross_harness_round_events(session_id, phase, round_number);

create index if not exists cross_harness_item_state_lookup
